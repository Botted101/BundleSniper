...
token = None
payload = [{ "itemType": "Bundle", "id": id } for id in settings["items"]]
cache = []
...
def get_product_id(id):
    try:
        conn = session.get(f"https://catalog.roblox.com/v1/bundles/{id}/details", verify=False)
        data = conn.json()
